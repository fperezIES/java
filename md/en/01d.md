...
'\R' is a regular expression that includes:
- \n (newline in Linux and macOS)
- \r\n (carriage return character followed by newline, used in Windows)
- \r (carriage return character only, previously used in systems like Mac OS)

This way, the scanner will only process whole lines, and we do not need to care about previous uses of the scanner.
