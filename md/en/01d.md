...
The right way of reading the data,  but wi will only get "Hello" instead of "Hello world" because the whitespace is the default delimiter.
...
If we need to read a whole line without caring about the current position of the cursor we can change the default delimiter, like this:
...
This way scanner will only process whole lines.
